...
def chat(chat_request: ChatRequest, current_user = Depends(get_current_user), db: Session = Depends(get_db)):
    try:
        ...
        # 获取当前分类，默认为"其他问题"
        current_category = chat_request.category if chat_request.category in category_responses else "其他问题"
        
        # 根据分类选择对应的回复模板
        responses = category_responses[current_category]
        
        # 根据用户输入生成不同的回复
        reply_index = hash(chat_request.user_input) % len(responses)
        reply = responses[reply_index]
